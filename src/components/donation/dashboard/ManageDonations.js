...
const ManageDonations = ({ organizationId }) => {
    ...
    useEffect(() => {
        const fetchDonations = async () => {
            try {
                ...
                const organizationDonations = data.filter(
                    (donation) => donation.organizationId === parseInt(organizationId)
                );
    ...
    return (
        <div className="manage-donations-container">
            ...
            {donations.length === 0 ? (
                <p>No donations found for this organization.</p>
            ) : (
                <ul className="donation-list">
                    {donations.map((donation) => (
                        <li className="donation-item" key={donation.id}>
